...
export function TermStructureChart() {
  ...
  const data = useMemo<TermData | null>(() => {
    ...
    if (term?.dte && term.dte.length >= 2) {
      return { labels: term.dte.map((d) => `${Math.round(d)}d`), dte: term.dte, today: term.iv, yday: null, avg5: null };
    }
    return null;
  }, [hist, term, replayingAt]);
...
function Plot({ width, height, data, hover, setHover }: {
  ...
  const extra = [...(data.yday ?? []), ...(data.avg5 ?? [])].filter((v): v is number => v != null);
